import { PoolEntityType, TokenInfo, usePoolsListQuery } from "queries/usePoolsListQuery"
import { findPoolForSwap, PoolMatchForSwap } from "queries/useQueryMatchingPoolForSwap"
...
const usePrices = () => {
    ...
    const coingeckoIds = useMemo(() => tokensList?.tokens.map(token => token.id), [tokensList?.tokens])
