...
export const useRefetchQueries = (queryKey?: string | Array<string>,
  ...
  return useCallback(async function refetchQueries(queryKeyArg?: string | Array<string>) {
    ...
    if (Array.isArray(queriesToRefetch)) {
      return Promise.all<any>(queriesToRefetch.map((query) => queryClient.refetchQueries(new RegExp(query) as unknown as string)))
    }

    return queryClient.refetchQueries(typeof queriesToRefetch === 'string' &&
