...
const MyPosition = ({ pair, myPositiionAmount, providedAssets }: Props) => {
  ...
  return (
    <HStack justifyContent="end">
      <Text align="right">
        {myPositiionAmount !== 'NA'
          ? `$${formatPrice(myPositiionAmount)}`
          : myPositiionAmount}
      </Text>
      <Tooltip
        label={
          <VStack>
            <Text>
              {asset1Name}: {fromChainAmount(asset1Amount, 6)}
            </Text>
            <Text>
              {asset2Name}: {fromChainAmount(asset2Amount, 6)}
            </Text>
          </VStack>
        }
        padding="20px"
